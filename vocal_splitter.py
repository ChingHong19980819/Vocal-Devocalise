...
def get_next_file():
	...
	for bn in [i for i in os.listdir(song_path) if not i.startswith('.') and os.path.isfile(song_path+'/'+i)]:
		...
		if not os.path.isfile(f'{song_path}/completed/{bn1}'):
			return bn
...
def main(argv):
	...
	while True:
		...
		if use_DNN:
			...
			ffmpeg_cmd = ['ffmpeg', '-i', (song_path+'/'+next_file), '-i', in_wav, '-i', out_wav_nonvocal, '-map', '0:v','-map', '1:a', '-map', '2:a', '-c:v', 'copy', '-c:a', 'aac', '-strict', 'experimental', "-n",  (song_path+'/completed/'+next_file)]
			subprocess.run(ffmpeg_cmd)
			os.remove((song_path+'/'+next_file))
