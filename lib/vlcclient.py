...
class VLCClient:
    ...
    def __init__(self, port=5002, path=None, qrcode=None, url=None):
        ...
        self.http_password = "ilovevsingforever"
        self.port = port
    ...
    def handle_zipped_cdg(self, file_path):
        ...
        if (mp3_file is not None) and (cdg_file is not None):
            ...
            raise Exception(
                "No .mp3 or .cdg was found in the zip file: " + file_path)
    ...
    def play_file(self, file_path, volume, params=[]):
        try:
            ...
            self.process = subprocess.Popen(
                command, stdin=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=subprocess.CREATE_NO_WINDOW)

            # wait for the process to start
            while self.process.poll() is not None:
                logging.info("We just finished something big here!")
                pass
    ...
    def command(self, command='', save_status=True):
        try:
            ...
            if not self.K.now_playing:
                ...
                self.K.now_playing_filename = unescape(
                    unescape(self.get_val_xml(request.text, "info name='filename'")))
                ...
                self.K.now_playing = self.K.filename_from_path(
                    self.K.now_playing_filename)
            return request
        except:
            logging.error(
                "No active VLC process. Could not run command: " + command)
            return SimpleNamespace(**{'text': self.last_status_text, 'status_code': 500})
    ...
    def parse_category(self, xml):
        def extract(t): return t[:min([t.find(c)
                                       for c in ['"', "'"] if c in t])]
        try:
            xml = xml.strip()
            name = extract(xml[16:])
    ...
    def get_stream_info(self, xml):
        try:
            info = self.get_val_xml(
                xml, 'information', '</information>').strip()
    ...
    def audio_track(self, track):
        return self.command(f"audio_track&val={track}")
    ...
    def stop(self):
        try:
            ...
            e = sys.exc_info()[0]
            logging.warn(
                f"Track stop: server may have shut down before http return code received: {e}")
            return
    ...
    # def foo():
    #     Thread(target=print("hello")).start()

    # Thread(target=setInterval(foo, 5)).start()
